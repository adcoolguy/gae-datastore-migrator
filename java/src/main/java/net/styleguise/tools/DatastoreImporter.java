...
public class DatastoreImporter extends RemoteDatastoreClient {
	...
	public static int DevRemoteApiPort = 8888;
	...
	public static void main(String[] args) throws Exception {
		...
		Path dataDir = Paths.get(console.readLine("CSV data dir (e.g. /Users/macbook/Dropbox/2share/gae-data-dump7440154122228127767): "));

		System.out.println("Enter your GAE credentials for administering gaej app >>>");
		String temp = console.readLine("Email (hit enter for local dev server): ");
		if(temp != null && !temp.equals("")) {
			email = temp;
		}
		temp = new String(console.readPassword("Password (hit enter for local dev server): "));
		if(temp != null && !temp.equals("")) {
			password = temp;
		}
		System.out.println("Enter host and port for the remote datastore");
		temp = console.readLine("Host (ex. APPNAME.appspot.com, just hit enter for local dev server): ");
		if(temp != null && !temp.equals("")) {
			Localhost = temp;
		}
		temp = console.readLine("Port (on GAE 443, just hit enter for local dev server): ");
		if(temp != null && !temp.equals("")) {
	...
	String parseFileNameForKind(Path csv){
		...
		int stopIndex = -1;
		if(fileName.indexOf(DatastoreExporter.Dash) > 0) {
			stopIndex = fileName.indexOf(DatastoreExporter.Dash);
			retVal = fileName.substring(0, stopIndex);
		} else {
			retVal = fileName;
		}
